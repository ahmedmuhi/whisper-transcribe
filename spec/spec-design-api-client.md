...
- **Transcription style**: the Clean (stored as `readability`) or Verbatim
  preference, one stored value shared by both MAI-Transcribe models. For
  MAI-Transcribe 1.5, Clean is selected by omitting `transcribeStyle` and only
  the literal `verbatim` value is serialized. MAI-Transcribe 2 always sends
  `enhancedMode.modelOptions.transcribeStyle` as `"clean"` or `"verbatim"`,
  because its own default is verbatim.
...
| Model, manual Target URI, and MAI style persistence | `Settings` | Returns `{ model, uri, transcribeStyle? }`; the optional field applies only to adapters that declare `supportsTranscribeStyle` |
...
|---|---|---|---|---|
| `mai-transcribe-2` | Azure MAI-Transcribe 2 | `https://cognitiveservices.azure.com/.default` | `STORAGE_KEYS.MAI_TRANSCRIBE_URI`, shared with 1.5 | WAV `audio` plus JSON `definition` with `enhancedMode.modelOptions.transcribeStyle` always set |
...
service detects the spoken language. Both MAI adapters accept the supported
source formats, convert to WAV through the existing worker/fallback, and
reject a converted payload at or above 250 MiB. Model adapters MUST NOT
...
1. `js/model-adapters/<model>.js` — frozen adapter object (pattern:
   `js/model-adapters/whisper.js`), including the UI metadata every
   user-visible surface is generated from: `label`, `optionLabel`, `uiOrder`,
   `maxUploadBytes`, `uploadLimitLabel`, `uploadLimitVerdict`,
   `uploadLimitAppliesTo`, and the `uri` row descriptor
   (`rowId`, `inputId`, `badgeId`, `title`, `subtitle`, `keywords`).
...
3. `js/constants.js` — `MODEL_TYPES`, `STORAGE_KEYS.<MODEL>_URI`,
   ...
   and the upload-limit constant the adapter declares.
4. Tests — adapter unit tests, settings persistence, hygiene expectations.
5. `tests/browser-live/` — guarded case plus protected Target URI
   (operator-gated).

Adapters that call the same Azure resource route share one `storageKeys.uri`
value and one frozen `uri` row object, so Settings renders a single Connection
row for them (both MAI adapters do this). An adapter that takes a transcription
style declares `supportsTranscribeStyle: true`; that flag, not a model id,
drives the Transcription style dropdowns and the `transcribeStyle` field in
`Settings.getModelConfig()`.

`index.html` and `js/selected-audio-controller.js` are no longer on the list:
both model `<select>`s and the Connection URI rows are generated by `Settings`,
the `ready-<id>` / `tooLarge-<id>` panels by `UI`, and the selection-time size
gate reads `maxUploadBytes` from the registry. A state with no generated panel
falls back to the `failed` panel with a warning rather than blanking the
workspace.
...
The MAI-Transcribe 1.5 adapter always serializes
`{"enhancedMode":{"enabled":true,"model":"mai-transcribe-1.5","task":"transcribe"}}`.
It adds `"transcribeStyle":"verbatim"` inside `enhancedMode` only when the
validated configuration exactly matches the verbatim enum value. Readability,
missing, and unknown values omit the field so Microsoft's readability default
and the existing request bytes are preserved.

The MAI-Transcribe 2 adapter always serializes
`{"enhancedMode":{"enabled":true,"model":"mai-transcribe-2","modelOptions":{"transcribeStyle":"clean"}}}`,
with `"verbatim"` in place of `"clean"` only when the validated configuration
exactly matches the verbatim enum value; readability, missing, and unknown
values send `"clean"`. It sends no `task`, `diarization`, `phraseList`,
`locales`, or timestamp options. Whisper and GPT Transcribe never receive a MAI
definition.
...
- **AC-001**: Given any registered model and a valid HTTPS Target URI, the
...
- **AC-005**: Every registered adapter satisfies the adapter contract in §4,
  and the registry preserves parser precedence.
...
- **AC-011**: MAI-Transcribe 1.5 verbatim requests add only
  `"transcribeStyle":"verbatim"` inside `enhancedMode`, and readability and
  unknown values omit the field. MAI-Transcribe 2 requests always carry
  `enhancedMode.modelOptions.transcribeStyle`: `"verbatim"` for verbatim and
  `"clean"` for everything else, with no `task` field. Whisper and GPT
  Transcribe remain unchanged.
...
- `tests/model-adapters.vitest.js` and `tests/response-parsers.vitest.js` — exact
  request/response contracts for every registered adapter, including both MAI
  style shapes (1.5 verbatim emission and readability omission, 2's always
  explicit style);
