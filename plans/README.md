...
| 035 | [Migrate the live two-model Azure contract to a least-privilege GitHub OIDC identity](https://github.com/ahmedmuhi/whisper-transcribe/issues/118) | P1 | L | 032 | IN PROGRESS (repository-only Steps 1–4 complete; stopped at the Step 5 fresh-approval checkpoint with no Step 5–9 operation performed. Verification: production and live-only builds passed; the production bundle excludes provider/token markers; 10 hygiene tests and 41 files / 523 tests passed with 93.25/82.97/93.95/93.25 coverage; lint, Knip, production Knip, high-severity audit, three size budgets, YAML parse, 11 deterministic Chromium cases, four-case live listing, absent-input four-case safe skip, fixture SHA-256, and diff checks passed. Independent review pinned every privileged-workflow action, corrected Azure Login cleanup configuration, and added fail-closed HTTPS/Azure-host validation before token egress. One known low-severity development-server `esbuild` advisory remains. Read-only primary-source recheck, 2026-07-18: Azure Login [`v3.0.0` is latest](https://github.com/Azure/login/releases/tag/v3.0.0), and [`@v3` guidance](https://github.com/Azure/login#readme) confirms OIDC, exact permissions, `allow-no-subscriptions`, `AZURE_CORE_OUTPUT=none`, and default post-cleanup; [GitHub requires mask-before-propagation](https://docs.github.com/en/actions/reference/workflows-and-actions/workflow-commands#masking-a-value-in-a-log), and [`az logout` removes CLI access](https://learn.microsoft.com/en-us/cli/azure/reference-index?view=azure-cli-latest#az-logout). Step 5 advisories: [`@v3` info-logs non-token OIDC claim metadata](https://github.com/Azure/login/blob/v3/src/common/LoginConfig.ts#L57-L83), so full logs must remain private; [GitHub's post-2026-07-15 immutable default `sub` rules](https://docs.github.com/en/actions/how-tos/secure-your-work/security-harden-deployments/oidc-in-azure) must be reconciled privately before federation.) |
| 036 | [Reconcile keyless documentation and add the sanitized operator runbook](https://github.com/ahmedmuhi/whisper-transcribe/issues/119) | P1 | L | 031–035 | DONE (documentation-only implementation independently reconciled against the final source and primary references; the agreed `CONTEXT.md` glossary was preserved byte-for-byte, active README/executor/design/API/FSM/ADR guidance was corrected, and the public-safe 16-stage operator runbook was added. Clean install/build, lint, 41 files / 523 tests with 93.25/83.01/93.95/93.25 coverage, both Knip gates, high-severity audit, exact 20 kB/54.88 kB/129-B size budgets, 11 deterministic Chromium cases, Markdown links, stale-term classification, sanitization, scope, and diff checks passed. The sole known low-severity development-server `esbuild` advisory remains. No live test or external configuration/resource/browser mutation occurred.) |
