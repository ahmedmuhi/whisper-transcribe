...
| 037 | [Qualify one immutable keyless release candidate across CI, Pages, and all 12 browser paths](https://github.com/ahmedmuhi/whisper-transcribe/issues/120) | P1 | L | 031–036 | IN PROGRESS (prerequisites reconciled read-only: SPA callbacks, User resource access, protected OIDC federation/roles, keyless workflow, and known-caller cleanup pass. Pages remains on legacy `main`; public SPA build variables, an immutable candidate ref, candidate Pages deployment, real-browser Microsoft sessions, live calls/probes, and the candidate OIDC rerun have not occurred. Candidate freeze follows the clean local baseline.) |
